...
def parse_file( fname, edges, polygons, csystems, screen, color ):
    temporary_polygons = new_matrix(4,0)
    temporary_edges = new_matrix(4,0)
    ...
    while c < len(lines):
        ...
        if line == 'push':
        	csystems.append([v[:] for v in csystems[-1]])
            ...
            add_sphere(temporary_polygons,
                       float(args[0]), float(args[1]), float(args[2]),
                       float(args[3]), step_3d)
            matrix_mult(csystems[-1],temporary_polygons)
            draw_polygons(temporary_polygons, screen, color)
            temporary_polygons = new_matrix(4,0)

        elif line == 'torus':
            #print 'TORUS\t' + str(args)
            add_torus(temporary_polygons,
                      float(args[0]), float(args[1]), float(args[2]),
            ...
            add_circle(temporary_edges,
                       float(args[0]), float(args[1]), float(args[2]),
                       float(args[3]), step)
            matrix_mult(csystems[-1],temporary_edges)
            draw_lines(temporary_edges, screen, color)
            temporary_edges = new_matrix(4,0)

        elif line == 'hermite' or line == 'bezier':
            #print 'curve\t' + line + ": " + str(args)
            add_curve(temporary_edges,
        ...
        # elif line == 'apply':
        #     matrix_mult( transform, temporary_edges )
        #     matrix_mult( transform, temporary_polygons )
